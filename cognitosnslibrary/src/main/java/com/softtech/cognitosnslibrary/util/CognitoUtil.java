package com.softtech.cognitosnslibrary.util;
...
import com.softtech.cognitosnslibrary.model.CognitoItemToDisplay;
...
public class CognitoUtil {
    ...
    private static CognitoUtil appHelper;
    ...
    private static List<CognitoItemToDisplay> currDisplayedItems;
    private static  int itemCount;

    private static List<CognitoItemToDisplay> trustedDevices;
    ...
    private static List<CognitoItemToDisplay> firstTimeLogInDetails;
    ...
    public static String userPoolId = null;
    public static String clientId = null;
    public static String clientSecret = null;
    public static Regions cognitoRegion = Regions.EU_CENTRAL_1;

    public static String snsPlatformApplicationArn = null;
    ...
    public static void init(Context context, CognitoConfigModel cognitoConfigModel) {
        ...
        if (appHelper == null) {
            appHelper = new CognitoUtil();
        }

        if (userPool == null) {
            userPool = new CognitoUserPool(context, cognitoConfigModel.getUserPoolId(), cognitoConfigModel.getClientId(), cognitoConfigModel.getClientSecret(), cognitoConfigModel.getCognitoRegion());
        }
        ...
        currDisplayedItems = new ArrayList<CognitoItemToDisplay>();
        trustedDevices = new ArrayList<CognitoItemToDisplay>();
        firstTimeLogInDetails = new ArrayList<CognitoItemToDisplay>();
    ...
    public static void setPool(Context context, CognitoConfigModel cognitoConfigModel){
        userPool = new CognitoUserPool(context, cognitoConfigModel.getUserPoolId(), cognitoConfigModel.getClientId(), cognitoConfigModel.getClientSecret(), cognitoConfigModel.getCognitoRegion());
    }
    ...
    public  static CognitoItemToDisplay getItemForDisplay(int position) {
        return  currDisplayedItems.get(position);
    }

    public static CognitoItemToDisplay getDeviceForDisplay(int position) {
        if (position >= trustedDevices.size()) {
            return new CognitoItemToDisplay(" ", " ", " ", Color.BLACK, Color.DKGRAY, Color.parseColor("#37A51C"), 0, null);
        }
        return trustedDevices.get(position);
    }

    public static CognitoItemToDisplay getUserAttributeForFirstLogInCheck(int position) {
    ...
    private static void refreshDisplayItemsForFirstTimeLogin() {
        ...
        for(Map.Entry<String, String> attr: firstTimeLogInUserAttributes.entrySet()) {
            ...
            CognitoItemToDisplay item = new CognitoItemToDisplay(attr.getKey(), attr.getValue(), message, Color.BLACK, Color.DKGRAY, Color.parseColor("#329AD6"), 0, null);
        ...
        for (String attr: firstTimeLogInRequiredAttributes) {
            if (!firstTimeLogInUserAttributes.containsKey(attr)) {
                CognitoItemToDisplay item = new CognitoItemToDisplay(attr, "", "Required", Color.BLACK, Color.DKGRAY, Color.parseColor("#329AD6"), 0, null);
                firstTimeLogInDetails.add(item);
                firstTimeLogInItemsCount++;
            }
        }
    ...
    public static void setDevicesForDisplay(List<CognitoDevice> devicesList) {
        ...
        for(CognitoDevice device: devicesList) {
            if (thisDevice != null && thisDevice.getDeviceKey().equals(device.getDeviceKey())) {
                ...
                CognitoItemToDisplay item = new CognitoItemToDisplay("", device.getDeviceName(), device.getCreateDate().toString(), Color.BLACK, Color.DKGRAY, Color.parseColor("#329AD6"), 0, null);
    ...
    private static void refreshWithSync() {
        ...
        for(String det: attributeDisplaySeq) {
            if(keySet.contains(det)) {
                ...
                CognitoItemToDisplay item = new CognitoItemToDisplay(signUpFieldsO2C.get(det), tempValues.get(tempKeys.indexOf(det)), "",
